...
import { setSonarrAnimeCategory } from "../../../store/actions/SonarrClientActions"
import { removeSonarrAnimeCategory } from "../../../store/actions/SonarrClientActions"
...
function SonarrAnimeCategory(props) {
  ...
  useEffect(() => {
    ...
    let previousAnimeNames = prevReduxState === undefined ? [] : prevReduxState.sonarr.animeCategories.map(x => x.name);
    let currentAnimeNames = reduxState === undefined ? [] : reduxState.sonarr.animeCategories.map(x => x.name);

    if (!(previousAnimeNames.length === currentAnimeNames.length && currentAnimeNames.every((value, index) => previousAnimeNames[index] === value)))
  ...
  const validateName = (value) => {
    ...
    if (!/\S/.test(value)) {
      newNameErrorMessage = "An anime category name is required.";
      newIsNameValid = false;
    } else if (/^[\w-]{1,32}$/.test(value)) {
      if (reduxState.sonarr.animeCategories.map(x => x.id).includes(props.category.id) && reduxState.sonarr.animeCategories.filter(c => typeof c.id !== 'undefined' && c.id !== props.category.id && c.name.toLowerCase().trim() === value.toLowerCase().trim()).length > 0) {
        newNameErrorMessage = "All anime categories must have different names.";
        newIsNameValid = false;
      } else if (reduxState.sonarr.categories.filter(c => c.name.toLowerCase().trim() === value.toLowerCase().trim()).length > 0) {
        newNameErrorMessage = "This name is already used by a TV category, anime and TV categories must have different names.";
        newIsNameValid = false;
      ...
      newNameErrorMessage = "Invalid category names, make sure they only contain alphanumeric characters, dashes and underscores. (No spaces, etc)";
      newIsNameValid = false;
    }
  ...
  const setCategory = (fieldChanged, data) => {
    dispatch(setSonarrAnimeCategory(props.category.id, fieldChanged, data));
  };

  const deleteCategory = () => {
    setIsOpen(false);
    setTimeout(() => dispatch(removeSonarrAnimeCategory(props.category.id), 150));
  };
              ...
              <Row>
                <Col lg="6">
                  <ValidatedTextbox
                    ...
                    placeholder="Enter anime category name"
                ...
                <Col lg="6">
                  <FormGroup className="custom-control custom-control-alternative custom-checkbox mb-3">
                    <Input
                      ...
                      id={"animeUseSeasonFolders" + props.category.id}
                      type="checkbox"
                    ...
                    <label
                      ...
                      htmlFor={"animeUseSeasonFolders" + props.category.id}>
...
export default SonarrAnimeCategory;
