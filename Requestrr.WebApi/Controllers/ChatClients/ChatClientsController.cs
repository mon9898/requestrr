    ...
    public class ChatClientsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAsync()
        {
            return Ok(new ChatClientsSettingsModel
            {
                ...
                AnimeRoles = _chatClientsSettings.Discord.AnimeRoles ?? Array.Empty<string>(),
        ...
        public async Task<IActionResult> SaveAsync([FromBody] ChatClientsSettingsModel model)
        {
            ...
            if ((model.AnimeRoles ?? Array.Empty<string>()).Any(x => !ulong.TryParse(x, System.Globalization.NumberStyles.Integer, null, out _)))
            {
                return BadRequest("Invalid anime roles, please make sure to enter the discord role ids.");
            }
            ...
            _chatClientsSettings.Discord.AnimeRoles = (model.AnimeRoles ?? Array.Empty<string>()).Where(x => !string.IsNullOrWhiteSpace(x)).Select(x => x.Trim()).ToArray();
